...
// Read from env vars (set in .env or at deploy time)
...
export async function getTimeline(start: string, end: string): Promise<TrackingEntry[]> {
...
export async function getTrend(start: string, end: string): Promise<ProductivityTrend[]> {
  const { data: entries } = await getSupabase()
    .from('tracking_entries')
...
export async function getWebEvents(start: string, end: string, siteId?: string): Promise<WebEvent[]> {
